...
@Component({
  ...
  styleUrls: ['./cuidado.component.css']
})
export class CuidadoComponent {
  ...
  ngOnInit(): void {
    this.anuncioService.getAnunciosByTipo("cuidado").subscribe(
      anuncios => {
        console.log('Anuncios:', anuncios);
  
        if (this.userData) {
          this.anuncios = anuncios.filter(anuncio => anuncio.autor && anuncio.autor.idUsuario !== this.userData?.idUsuario);
        } else {
  ...
  reservar(anuncio: Anuncio) {
    this.confirmationService.confirm({
      message: `Estás a punto de reservar el anuncio de ${anuncio.autor.nombre} ${anuncio.autor.apellido1} con hora de inicio ${anuncio.hora_inicio} y hora de fin ${anuncio.hora_fin}. ¿Estás seguro?`,
      ...
      accept: () => {
        ...
        this.anuncioService.updateAnuncioById(anuncio.idAnuncio, anuncio).subscribe(
          () => {
            console.log('Reserva realizada en:', anuncio);
  ...
  paginar(event: any) {
    ...
    console.log("ANUNCIOS MOSTRADOS", this.anunciosMostrados);
  }
  ...
  recargarPagina() {
    this.anuncioService.getAnunciosByTipo("cuidado").subscribe(
      anuncios => {
        this.anuncios = anuncios;
        console.log('Página recargada:', anuncios);
      },
      (error) => {
        console.error('Error al obtener anuncios:', error, 'Anuncios:', this.anuncios);
      }
    );
  }
